...
describe("io.parse_markdown", function()
  ...
  describe("P0-1: sort_section_tasks でサブ見出し内タスクが崩壊しない", function()
    ...
    it("ソート後も「仕事」配下に 3 件のタスクが残る", function()
      ...
      assert.equals(3, #shigoto_tasks,
        "「仕事」配下のタスクが 3 件のはずが " .. #shigoto_tasks .. " 件（サブセクション崩壊）")
    end)
    ...
    it("ソート後も「プライベート」配下に 2 件のタスクが残る", function()
      ...
      assert.equals(2, #private_tasks,
        "「プライベート」配下のタスクが 2 件のはずが " .. #private_tasks .. " 件（サブセクション崩壊）")
    end)
    ...
    it("「仕事」ブロック内では due 昇順にソートされる", function()
      ...
      if #shigoto_tasks >= 3 then
        ...
        assert.equals("2025-01-01", shigoto_tasks[1].due,
          "仕事ブロック1番目の due が最小のはず")
        assert.equals("2025-02-01", shigoto_tasks[2].due,
          "仕事ブロック2番目の due が中間のはず")
        assert.equals("2025-03-01", shigoto_tasks[3].due,
          "仕事ブロック3番目の due が最大のはず")
      end
    end)
    ...
    it("「プライベート」ブロック内では due 昇順にソートされる", function()
      ...
      if #private_tasks >= 2 then
        assert.equals("2025-01-15", private_tasks[1].due,
          "プライベートブロック1番目の due が最小のはず")
        assert.equals("2025-02-15", private_tasks[2].due,
          "プライベートブロック2番目の due が最大のはず")
      end
    end)

    -- テスト 5: サブセクションの並び順がソートで変わらない
    it("サブセクションの並び順がソート後も「仕事」→「プライベート」のまま", function()
      local data = make_data_with_subsections()
      local sorted = logic_mod.sort_section_tasks(data.sections["Today"])

      -- ネスト構造の場合: subsections の順序を直接確認
      if type(sorted) == "table" and sorted.subsections then
        assert.equals("仕事", sorted.subsections[1].name,
          "「仕事」が先に現れるべき")
        assert.equals("プライベート", sorted.subsections[2].name,
          "「プライベート」が後に現れるべき")
      else
        -- フラット構造: ### 行の登場順を確認する
