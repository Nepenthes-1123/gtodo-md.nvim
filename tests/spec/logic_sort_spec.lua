...
describe("logic.sort_section_tasks general sorting rules", function()
	...
	it("完了済みタスクは未完了タスクより下になる", function()
		local sec = make_items("- [x] 完了タスク due:2020-01-01", "- [ ] 未完了タスク")
	...
	it("dueありタスクはdueなしタスクより上になる", function()
		local sec = make_items("- [ ] dueなし", "- [ ] dueあり due:2025-01-01")
		local sorted = logic_mod.sort_section_tasks(sec)
	...
	it("due日付の古い順(昇順)になる", function()
		local sec = make_items("- [ ] タスク2 due:2025-02-01", "- [ ] タスク1 due:2025-01-01")
		local sorted = logic_mod.sort_section_tasks(sec)
	...
	it("dueが同じ場合、優先度順になる", function()
		local sec = make_items("- [ ] (B) タスクB due:2025-01-01", "- [ ] (A) タスクA due:2025-01-01")
		local sorted = logic_mod.sort_section_tasks(sec)
	...
	it("dueも優先度も同じ場合、元の順序が維持される(stable sort)", function()
		local sec = make_items("- [ ] タスクB due:2025-01-01", "- [ ] タスクA due:2025-01-01")
		local sorted = logic_mod.sort_section_tasks(sec)
