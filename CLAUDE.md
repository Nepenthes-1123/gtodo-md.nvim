...
- `config.lua` — デフォルト値と `M.options`/`M.get(key)` アクセサ、およびセクション名の正本（`config.sections.TODAY/NEXT/WAITING/SOMEDAY`）。`setup({sections=...})`(#94)でキー単位に上書きできる(部分上書き可)。`M.default_sections`は変更されない固定のデフォルト名で、`M.section_aliases(key)`はそのキーに対して現在有効な名称候補([カスタム名, デフォルト名]、未カスタマイズなら[デフォルト名]のみ)を返す — デフォルト名は常にエイリアスとして受理され続けるため、カスタム化後も既存ファイルの見出しを手動でリネームする必要がない(`io.lua`の`parse_markdown`がその場で正規化し、次回保存時にカスタム名で書き戻る。`init.lua`の`BufWritePre`バリデーションも`section_aliases`経由でカスタム名・デフォルト名の両方を許容する)。
