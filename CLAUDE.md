...
- `init.lua` — エントリーポイント（`M.setup`）。config・autocmd・keymap・ユーザーコマンドをここで結線する。キーマップ/コマンドから呼ばれる2つの大きな「適応的」エントリーポイント、`add_or_edit_task()` と `sort_and_check_dues()`、および `BufEnter` 時の自動処理 `handle_buf_enter()` もここにある。autocmd登録の実体は `autocmds.lua` へ移してあり、`init.lua` に残る `M.setup_autocmds()` は公開エントリポイントを維持するための薄い転送関数（既存のテストスペックが直接呼ぶため削除しないこと）。
